...
// Updated theme config values matching the reference picture palette:
// [Glossy Medium Blue, Deep Navy, Glossy Light Grey, Glossy White]
const X = {
  count: 200,
  colors: [0x120f17, 0x120f17, 0x301eb6, 0xd2d2d2, 0x7a7a7a],
...
class Z extends d {
  ...
  setColors(e) {
    if (Array.isArray(e) && e.length > 0) {
      // Instantiates distinct instances instead of drawing from a linear gradient
      const threeColors = e.map(col => new l(col));
      
      for (let idx = 0; idx < this.count; idx++) {
        let pickedColor;
        
        if (idx === 0) {
          // Keep control sphere connected to the initial index configuration color
          pickedColor = threeColors[0];
          this.light.color.copy(pickedColor);
        } else {
          // Mix distribution dynamically among the requested theme variations
          const rand = Math.random();
          if (rand < 0.3) {
            pickedColor = threeColors[0]; // Dark grey/black (~30%)
          } else if (rand < 0.5) {
            pickedColor = threeColors[2] || threeColors[0]; // Deep purple/blue (~20%)
          } else if (rand < 0.7) {
            pickedColor = threeColors[3] || threeColors[0]; // Light grey (~20%)
          } else if (rand < 0.85) {
            pickedColor = threeColors[4] || threeColors[0]; // Medium grey (~15%)
          } else {
            pickedColor = threeColors[1] || threeColors[0]; // Dark grey/black duplicate (~15%)
          }
        }
        
        this.setColorAt(idx, pickedColor);
      }
...
const Ballpit = ({ className = '', followCursor = true, colors, ...props }) => {
  ...
  useEffect(() => {
    ...
    const palette = Array.isArray(colors) && colors.length
      ? colors
      : [0x120f17, 0x120f17, 0x301eb6, 0xd2d2d2, 0x7a7a7a];
